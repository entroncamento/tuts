...
def contains_loose(haystack: str, needle: str) -> bool:
    ...
    if n in h:
        return True
    # Para critérios longos, aceita cobertura parcial de termos informativos.
    tokens = [t for t in re.findall(r"[a-zA-Z0-9_]+", n) if len(t) >= 4]
    if len(tokens) < 4:
...
def automatic_checks(item: DatasetItem, resposta: str, tuts: TUTSResult) -> dict[str, Any]:
    ...
    must_not_violations = [bad for bad in item.must_not_include if contains_loose(resposta, bad)]
...
def write_human_review_csv(path: Path, results: list[dict[str, Any]]) -> None:
    ...
    fields = [
        ...
        "missing_must_include", "must_not_violations", "citation_count", "resposta_preview", "comentario_humano", "nota_humana_final",
    ]
...
def write_markdown_report(path: Path, output: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Missing citations in in-scope items: {r['n_sem_citacao_em_in_scope']}",
        f"- Must-not violations: {r['n_must_not_violations']}",
        "",
        "## Human review flags",
    ]
...
def print_final_report(quality: dict[str, Any], robustness: dict[str, Any], duracao_s: int) -> None:
    ...
    for key in [
        ...
        "n_truncagem_suspeita", "n_cache_hits", "n_sem_citacao_em_in_scope", "n_must_not_violations",
