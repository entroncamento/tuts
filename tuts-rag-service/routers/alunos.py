...
async def perguntar(
    ...
    # ⚡ Procura no Redis Vector Search
    ...
    if resposta_cacheada:
        async def cached_generator():
            ...
            yield sse({"status_msg": "⚡ Déjà vu! Fui buscar esta resposta à memória de longo prazo do Redis."})
    ...
    async def event_generator():
        ...
        # 1. INTENÇÃO DINÂMICA E EXPANSÃO (Reativada! 🚀)
        ...
        try:
            # Tenta usar a IA para expandir a pergunta (gerar sinónimos)
            ...
            # 🛡️ FALLBACK DE SEGURANÇA: Se a expansão levar com um 429, não quebra o sistema!
            logger.warning(f"Expansão de queries falhou (possível 429). A usar query original. Erro: {e}")
            queries_pesquisa = [texto_final_aluno]

        # ⏳ O TRUQUE ANTI-429: O "Tactical Delay"
        # Obriga o Python a esperar 2.5 segundos. Isto dá tempo para o Rate Limiter da 
        # API da faculdade "arrefecer" e esquecer o primeiro pedido, deixando o segundo passar limpo.
        yield sse({"status_msg": "🧠 A mapear conexões e sinónimos da matéria..."})
        ...
        try:
            ...
            # Recupera a mensagem dinâmica para mostrar o 429 se for esse o caso
            texto_erro = str(exc) if "limite" in str(exc) or "API" in str(exc) else "Erro de comunicação com o servidor de Inteligência Artificial."
            erro_msg = f"\n\n❌ {texto_erro}"
