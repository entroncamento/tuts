...
from fastapi import APIRouter, File, Form, Request, UploadFile, Header, HTTPException, BackgroundTasks
...
class GeradorPensamentos:
    ...
    def avaliacao(num_finais, modo):
        if num_finais == 0:
            return random.choice([
                "Não encontrei referências exatas nos PDFs. Vou usar o meu conhecimento geral.",
                "Os apontamentos não falam sobre isto. A transitar para a minha base de dados global.",
            ])
        return random.choice([
            f"Filtrei o ruído. Fiquei com os {num_finais} parágrafos mais perfeitos.",
...
async def perguntar(
    ...
    async def event_generator():
        ...
        try:
            ...
            logger.warning(f"Expansão de queries falhou. A usar query original. Erro: {e}")
        ...
        if not houve_erro and full_response and len(full_response.strip()) > 20:
            ...
            if message_id:
                ...
                logger.warning("Não foi possível agendar análise: message_id está ausente.")
